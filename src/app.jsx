...
function ClubFinder(){
  ...
  const venueTypeCounts = React.useMemo(() => {
    const counts = { outdoor: 0, indoor: 0, public: 0 };
    const clubs = allClubsLocal;
    for (const club of clubs) {
      if (!club) continue;
      const sportMatch = activeSport ? club.sport === activeSport : true;
      ...
      // Public: simple boolean flag on the club
      if (club.public) counts.public += 1;

      // Indoor / Outdoor: reuse same inference logic as the filter
      const ctRaw = club.court_type && club.court_type.toString().trim();
      const ct = ctRaw ? ctRaw.toLowerCase() : '';
      const hasIndoorProp = Object.prototype.hasOwnProperty.call(club, 'indoor');
      const indoorVal = hasIndoorProp ? club.indoor : undefined;

      const isOutdoor = (() => {
        if (ct === 'outdoor') return true;
        if (!ct && indoorVal === false) return true;
        if (ct.includes('outdoor') && !ct.includes('indoor')) return true;
        return false;
      })();

      const isIndoor = (() => {
        if (ct === 'indoor' || ct === 'indoor/outdoor') return true;
        if (ct.includes('indoor')) return true;
        if (!ct && indoorVal === true) return true;
        return false;
      })();

      if (isOutdoor) counts.outdoor += 1;
      if (isIndoor) counts.indoor += 1;
    }
    return counts;
  }, [allClubsLocal, activeSport]);
  ...
  const imgSrc = customFav || derivedFav;
    ...
    const actionsHtml = `<div class=\"mt-3 flex flex-wrap justify-end gap-2\">${(typeof club.lat === 'number' && typeof club.lng === 'number') ? `<a href=\"https://www.google.com/maps/search/?api=1&query=${club.lat},${club.lng}\" target=\"_blank\" rel=\"noopener noreferrer\" class=\"inline-flex items-center gap-1.5 text-[12px] font-semibold px-3 py-1.5 rounded-md bg-teal-500 text-white hover:bg-teal-600\" style=\"color:#fff !important; background:#14b8a6; text-decoration:none; padding:6px 12px; border-radius:6px; font-weight:600; font-size:12px;\">Get directions</a>` : ''}${normalizedWebsiteUrl ? `<a href=\"${normalizedWebsiteUrl}\" target=\"_blank\" rel=\"noopener noreferrer\" aria-label=\"Open website\" title=\"Open website\" class=\"inline-flex h-8 w-8 items-center justify-center rounded-md bg-slate-100 text-slate-600 hover:bg-teal-600 hover:text-white transition\" style=\"text-decoration:none;\">${window.WebsiteIcon ? '<svg viewBox=\"0 0 24 24\" class=\"block h-[18px] w-[18px] translate-x-[1px]\" fill=\"none\" stroke=\"currentColor\" stroke-width=\"1.8\" stroke-linecap=\"round\" stroke-linejoin=\"round\" aria-hidden=\"true\"><path d=\"M12 21a9 9 0 1 0 0-18 9 9 0 0 0 0 18z\"></path><path d=\"M3.6 9h16.8\"></path><path d=\"M3.6 15h16.8\"></path><path d=\"M12 3a15 15 0 0 1 4 8 15 15 0 0 1-4 8 15 15 0 0 1-4-8 15 15 0 0 1 4-8z\"></path></svg>' : ''}</a>` : ''}</div>`;
    const popupHtml = `
      <div class=\"p-2\">
